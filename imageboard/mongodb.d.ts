...
//    Additional type annotations by Sigma Systems
...
declare module "mongodb" {
    ...
    export class MongoClient{
        ...
        public open(callback: (err : Error, mongoclient : MongoClient) => void): void;
        public close(callback?: (err : Error, result : any) => void): void;
    ...
    export class Server {
        ...
        public connect(dbInstance: Db, options: any, callback: (err: Error, xx: any, server: Server) => void): any;
        public close(callback?: () => void): void;
    }

    import events = require('events');
    // Class documentation : http://mongodb.github.io/node-mongodb-native/api-generated/db.html
    export class Db extends events.EventEmitter {
    ...
    export class Binary {
        constructor (buffer: any/*Buffer*/, subType?: number);
        ...
        read(position: number, length: number): any/*Buffer*/;
    ...
    export interface ServerOptions {
        ...
        socketOptions?: any;
        // - emit an 'open' event when opened, default: true
        emitOpen?: boolean;
        // - use ssl connection (needs to have a mongod server with ssl support, default:false
        ssl?: boolean;
        // - validate mongod server certificate against ca (needs to have a mongod server with ssl support, 2.4 or higher), default:false
        sslValidate?: boolean;
        // Array of valid certificates either as Buffers or Strings (needs to have a mongod server with ssl support, 2.4 or higher), default:null
        sslCA?: Array<any>;
        // - String or buffer containing the certificate we wish to present (needs to have a mongod server with ssl support, 2.4 or higher), default:null
        sslCert?: string;
        // - String or buffer containing the certificate private key we wish to present (needs to have a mongod server with ssl support, 2.4 or higher), default:null
        sslKey?: string;
        // - String or buffer containing the certificate password (needs to have a mongod server with ssl support, 2.4 or higher), default:null
        sslPass?: string;
        // - an object representing a logger that you want to use, needs to support functions debug, log, error
        // **({error:function(message, object) {}, log:function(message, object) {}, debug:function(message, object) {}})**.,default:null
        logger?: any;
        // - force the server to error if the BSON message is to big, default:false
        disableDriverBSONSizeCheck?: boolean;
    }
